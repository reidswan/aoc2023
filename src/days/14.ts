...
const parseInput = (src: string): RockMap => {
  ...
  Object.values(rockMap.squareRocksByCol).forEach((it) =>
    it.sort((a, b) => a.y - b.y),
  );
  Object.values(rockMap.squareRocksByRow).forEach((it) =>
    it.sort((a, b) => a.x - b.x),
  );
...
const rollNorth = (rockMap: RockMap): RockMap => {
  ...
  const roundRocks = _.keys(roundByCol)
    .map((col) => {
...
const rollWest = (rockMap: RockMap): RockMap => {
  ...
  const roundRocks = _.keys(roundByRow)
    .map((row) => {
...
const calculateLoadWeight = (rockMap: RockMap): number => {
  return rockMap.roundRocks.reduce(
    (acc, { y }) => acc + rockMap.gridHeight - y,
    0,
  );
};
