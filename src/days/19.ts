...
export const solve = () => {
  ...
  const accepted = input.components.filter(
    (component) =>
      runThroughWorkflows(component, input.workflows) === FinalResult.Accept,
  );

  const propertySum = accepted
    .map((c) => c["x"] + c["m"] + c["a"] + c["s"])
    .reduce(_.add);
  console.log("Part 1:", propertySum);

  const acceptedCombos = findAcceptedRanges(input.workflows)
    .map(calculateRangeSize)
    .reduce(_.add, 0);
...
const parseInput = (
  s: string,
): { components: Component[]; workflows: WorkflowMap } => {
  const parts = s.split("\n\n");
  if (parts.length !== 2) throw new Error("invalid format");

  const [workflowsS, componentsS] = parts;

  const workflows = workflowsS
    .split("\n")
    .map(parseWorkflow)
    .reduce((wfs, wf) => ({ ...wfs, [wf.id]: wf }), {});
  const components = componentsS.split("\n").map(parseComponent);

  return { workflows, components };
};

const runThroughWorkflows = (
  component: Component,
...
const parseRule = (s: string): WorkflowRule => {
  const res =
    /(?<property>[xmas])(?<condition>[<>])(?<threshold>[0-9]+):(?<result>[a-zA-Z]+)/.exec(
      s,
    );
  if (!res || !res.groups) {
...
const findAcceptedRanges = (workflows: WorkflowMap): ComponentRange[] => {
  ...
  while (stack.length) {
    ...
    for (const rule of workflow.rules) {
      ...
      const { success, failure } = splitRange(
        currRange[rule.property],
        rule.condition,
        rule.threshold,
      );
...
const splitRange = (
  ...
  if (condition === Condition.Greater) {
    return {
      success: { min: threshold + 1, max: range.max },
      failure: { min: range.min, max: threshold },
    };
  } else {
    return {
      success: { min: range.min, max: threshold - 1 },
      failure: { min: threshold, max: range.max },
    };
  }
};
