...
export const solve = () => {
  ...
  const total = patterns
    .map((p) => countFromReflection(p, findReflectionInHashes))
    .reduce(_.add);
  console.log("Part 1:", total);

  const totalWithSmudge = patterns
    .map((p) => countFromReflection(p, findReflectionInHashesWithSmudge))
    .reduce(_.add);
...
const parsePattern = (src: string): Pattern => {
  ...
  const columnHashes = _.range(grid[0]?.length || 0)
    .map((i) => grid.map((g) => g[i]))
    .map(hash);
...
const countFromReflection = (
  pattern: Pattern,
  findReflection: (hashes: number[]) => number | undefined,
): number => {
  let found;
  ...
  if (found) {
    const reflectionPoint =
      pattern.rowHashes.length - Math.floor(found / 2) - 1;
    return 100 * reflectionPoint;
  }
  ...
  if (found) {
    const reflectionPoint =
      pattern.columnHashes.length - Math.floor(found / 2) - 1;
    return reflectionPoint;
  }
...
const findReflectionInHashesWithSmudge = (
  hashes: number[],
): number | undefined => {
  // A reflection goes to the edge of the pattern, so find indexes with matching hashes
  if (!hashes.length) return;
...
const checkMatch = (
  ...
  for (
    let offset = 0;
    offset < Math.ceil((lowerIndex - upperIndex) / 2);
    offset++
  ) {
