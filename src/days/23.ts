...
const longestPathP1 = (map: TileType[][], start: Coord, end: Coord): number => {
    const queue = [{ loc: start, dist: 0, path: new Set<Coord>(hashCoord).add(start) }]
    const mb: number[] = []
    ...
    while (queue.length) {
        ...
        if (nexts.length === 1) {
            ...
            queue.push(...nexts.map(next => ({ loc: next, path: path.clone().add(loc), dist: dist + 1 })))
        }
    }
...
const nextTiles = (map: TileType[][], { x, y }: Coord, path: Set<Coord>): Coord[] => {
...
const longestPathP2 = (map: TileType[][], start: Coord, end: Coord): number => {
    ...
    const queue = [{ curr: startID, dist: 0, visited: new Set<NodeID>(x => x.toString()).add(startID) }]
    let maxDist = 0;
    while (queue.length) {
        ...
        edges.get(curr)!.filter(a => !visited.has(a.to)).forEach(({ to, dist: dist_ }) => {
            queue.push({ curr: to, dist: dist + dist_, visited: visited.clone().add(curr) })
        })
    }
...
const pathGraph = (map: TileType[][]): { nodes: Map_<NodeID, Coord>, edges: Map_<NodeID, { to: NodeID, dist: number }[]> } => {
    const visited = new Set<Coord>(hashCoord);
    const nodes = new Map_<NodeID, Coord>(n => n.toString());
    const edges = new Map_<NodeID, { to: NodeID, dist: number }[]>(n => n.toString());
    map.forEach((row, y) => row.forEach((elm, x) => {
