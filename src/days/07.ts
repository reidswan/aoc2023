...
export const solve = () => {
  ...
  hands.forEach(
    (hand) => (hand.type = determineHandType(hand.cards, (s) => s === "J")),
  );
...
const cmpHands =
  ...
  (hand1: Hand, hand2: Hand): number => {
    ...
    const firstNonMatch = _.zip(hand1.cards, hand2.cards).find(
      ([a, b]) => a !== b,
    );
