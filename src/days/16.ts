...
const maxEnergizedCellCount = (grid: TileType[][]): number => {
    ...
    grid.forEach((row, y) => {
        ...
        allStarts.push({ location: { x: row.length, y }, direction: Direction.Left })
    })
    grid[0].forEach((_, x) => {
        allStarts.push({ location: { x, y: -1 }, direction: Direction.Down })
        allStarts.push({ location: { x, y: grid.length }, direction: Direction.Up })
    })

    return Math.max(...allStarts.map(start => countEnergizedCells(grid, start)))
}
...
const countEnergizedCells = (grid: TileType[][], start: Beam): number => {
    ...
    let beams = [start]
    while (beams.length) {
        beams = beams.map(beam => transitionOnce(beam, grid).filter(beam => !seen.has(beam))).flat()
        beams.forEach(beam => {
    ...
    return cellEnergized.reduce((acc, row) => acc + row.filter(isEnergized => isEnergized).length, 0)
}
