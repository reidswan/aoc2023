...
export const solve = () => {
  const grid = readInputGrid("input/day17.txt").map((row) =>
...
const computeMinHeatLoss = (
  grid: number[][],
  minSteps: number,
  maxSteps: number,
): number => {
  const end = { x: grid[0].length - 1, y: grid.length - 1 };
  const queue: HeapQueue<QueueEntry> = new HeapQueue(
