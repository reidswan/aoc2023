...
const parseInput = (grid: string[][]): GardenMap => {
    ...
    const locations = grid.map((row, y) => row.map((entry, x) => {
        switch (entry) {
            case '#':
...
const step = (locations: PlotType[][], from: Set<Coord>, getLoc: (grid: PlotType[][], coord: Coord) => PlotType | undefined): Set<Coord> => {
    const res = new Set<Coord>(hashCoord)
    from.values()
        .map(c => neighbors(c).filter(c2 => getLoc(locations, c2) === PlotType.Garden))
        .flat()
...
const countLocationsForStepsP2 = (gardenMap: GardenMap): number => {
    const shortestDists = computeShortestDistances(gardenMap.locations, gardenMap.start)
    const [evens, odds] = _.partition(shortestDists.items(), ([, dist]) => dist % 2 === 0)
    const evenCorners = evens.filter(([, dist]) => dist > 65).length
    const oddCorners = odds.filter(([, dist]) => dist > 65).length
    const n = (26501365 - Math.floor(gardenMap.locations.length / 2)) / gardenMap.locations.length
    return ((n + 1) * (n + 1)) * odds.length + (n * n) * evens.length - (n + 1) * oddCorners + n * (evenCorners - 1)
}
