...
const makeStack = (bricks: Brick[]): BrickStack => {
  ...
  for (const brick of bricks) {
    ...
    for (
      ...
      currHeight <= height + brick.end.z - brick.start.z;
      currHeight++
    ) {
...
const hasXYOverlap = (b1: Brick, b2: Brick): boolean => {
  ...
  return (
    overlap(b1StartX, b1EndX, b2StartX, b2EndX) &&
...
const countDisintegratable = ({
  supports,
  supportedBy,
}: BrickStack): number => {
  return _.values(supports).filter((supportedBricks) =>
    supportedBricks.every((it) => supportedBy[it].length > 1),
  ).length;
};
  ...
  stack.reverse().forEach((row) => {
    row.forEach((brick) => {
      ...
      while (queue.length) {
        ...
        const falling = supports[next].filter(
          (supported) =>
            supportedBy[supported].filter((it) => !fallen.has(it)).length === 0,
        );
        queue.push(...falling);
