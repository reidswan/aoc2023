...
export const findDistances = (
  ...
  const emptyRows = _.range(maxRow).filter(
    (row) => !spaceMap.galaxyLocations.some((l) => l.x === row),
  );
  const emptyCols = _.range(maxCol).filter(
    (col) => !spaceMap.galaxyLocations.some((l) => l.y === col),
  );
  ...
  for (let from = 0; from < spaceMap.galaxyLocations.length; from++) {
    for (let to = from + 1; to < spaceMap.galaxyLocations.length; to++) {
      ...
      const [xStart, xEnd] = [
        Math.min(fromGalaxy.x, toGalaxy.x),
        Math.max(fromGalaxy.x, toGalaxy.x),
      ];
      const [yStart, yEnd] = [
        Math.min(fromGalaxy.y, toGalaxy.y),
        Math.max(fromGalaxy.y, toGalaxy.y),
      ];
      let distance = xEnd - xStart + (yEnd - yStart);
      distance +=
        emptyRows.filter((row) => xStart < row && row < xEnd).length *
        (expansionFactor - 1);
      distance +=
        emptyCols.filter((col) => yStart < col && col < yEnd).length *
        (expansionFactor - 1);
