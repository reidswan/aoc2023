...
const applyInstruction = (map: Map<number, Lens[]>, instruction: Instruction): Map<number, Lens[]> => {
    ...
    switch (instruction.operation) {
        case Operation.Remove:
            map.set(destinationBox, map.get(destinationBox)!.filter(it => it.label !== instruction.label))
            break
        case Operation.Insert:
            map.set(destinationBox, addOrReplaceLens(map.get(destinationBox)!, instruction))
    }
...
const computeFocusingPower = (map: Map<number, Lens[]>): number => {
    return [...map.entries()].reduce((acc, [box, lenses]) =>
        acc + lenses.reduce((acc1, lens, index) => acc1 + (box + 1) * (index + 1) * lens.focalLength, 0),
        0)
}
