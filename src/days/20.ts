...
export const solve = () => {
  ...
  const totals = _.range(1000).reduce(
    (totals) => {
...
const handleButtonPress = (
  ...
  while (queue.length) {
    ...
    if (resultPulse !== undefined) {
      queue.push(
        ...module.outgoingEdges.map((edge) => ({
          modName: edge,
          incomingPulse: resultPulse,
          from: modName,
        })),
      );
    }
...
const findRxCycleLength = (modGraph: ModuleGraph): number => {
  const finalConj = _.values(modGraph).find((it) =>
    it.outgoingEdges.includes("rx"),
  )!;

  return finalConj.incomingEdges
    .map((e) => findCycleLength(modGraph, e))
