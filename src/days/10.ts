...
const getLoopStartingAt = (
  ...
  while (!get(seen, curr)) {
    ...
    const next = currPipe.connects.filter(
      (c) => c.x != prev.x || c.y != prev.y,
    )[0];
    if (next === undefined) {
...
const getLoop = (pipeMap: PipeMap): Coord[] => {
  ...
  return filterMap(start.connects, (next) =>
    getLoopStartingAt(pipeMap, pipeMap.start, next),
  )[0];
};
...
const findBoundedByLoop = (pipeMap: PipeMap, loop: Coord[]): number => {
  ...
  for (let y = 0; y < pipeMap.pipes.length; y++) {
    ...
    for (let x = 0; x < row.length; x++) {
      ...
      for (
        let x1 = x + 1, y1 = y + 1;
        x1 < row.length && y1 < pipeMap.pipes.length;
        x1++, y1++
      ) {
        ...
        if (isLoopTile(loop, coord)) {
          ...
          if (
            pipe.pipeType !== PipeType.NEBend &&
            pipe.pipeType !== PipeType.SWBend
          ) {
            inLoop = !inLoop;
          }
        }
