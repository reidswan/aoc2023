...
const parseInput = (s: string): NodeMap => {
    ...
    const nodes: LinkMap = {}
    nodesRaw.split("\n").filter(it => it !== "").forEach(line => {
        const match = /(?<name>[0-9A-Z]+)\s*=\s*\((?<left>[0-9A-Z]+),\s*(?<right>[0-9A-Z]+)\)/.exec(line)
        if (!match || !match.groups) {
            throw new Error(`unexpected format for line ${line}`)
        }

        nodes[match.groups["name"]] = { left: match.groups["left"], right: match.groups["right"] }
    })

    return {
        instructions: instructions.split("").filter(it => it == "L" || it == "R").map(parseInstruction),
        nodes,
    }
}
...
const traverse = (start: string, nodeMap: NodeMap, isEnd: (node: string) => boolean): { end: string, count: number } => {
    let curr = start;
    let count = 0;
    do {
        const instruction = nodeMap.instructions[count % nodeMap.instructions.length]
...
const move = (from: string, instruction: Instruction, nodes: LinkMap): string => {
...
const solveP2 = (nodeMap: NodeMap): number => {
    ...
    const counts = nodes.map(node => traverse(node, nodeMap, it => it.endsWith("Z")).count)
