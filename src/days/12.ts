...
export const solve = () => {
    ...
    let total = input.reduce((acc, record) => acc + countPossibleConditions(record), 0)
    console.log("Part 1:", total)

    total = input.reduce((acc, record) => acc + countPossibleConditions(unfoldRecord(record)), 0)
    ...
    return src.map(line => {
        ...
        return {
            conditions: conditions.split("").map(c => {
                switch (c) {
                    case ".": return Condition.Working
                    case "#": return Condition.Broken
                    case "?": return Condition.Unknown
                }
                throw new Error(`unknown condition: ${c}`)
            }),
            brokenSpringAudit: brokenSprings.split(",").filter(isNumeric).map(it => parseInt(it))
        }
    })
}
...
const unfoldRecord = (record: ConditionRecord): ConditionRecord => {
    return {
        conditions: [...record.conditions, Condition.Unknown, ...record.conditions, Condition.Unknown, ...record.conditions, Condition.Unknown, ...record.conditions, Condition.Unknown, ...record.conditions],
        brokenSpringAudit: [...record.brokenSpringAudit, ...record.brokenSpringAudit, ...record.brokenSpringAudit, ...record.brokenSpringAudit, ...record.brokenSpringAudit]
    }
}
...
const countPossibleConditions = cachedFn(({ brokenSpringAudit, conditions }: ConditionRecord): number => {
    ...
    switch (_.head(conditions)!) {
        ...
        case Condition.Broken:
            if (conditions.slice(0, run).every(it => it !== Condition.Working) && conditions[run] !== Condition.Broken) {
                // A match, followed by a separator!
                return countPossibleConditions({ conditions: conditions.slice(run + 1), brokenSpringAudit: rest })
            }
            return 0
...
const matches = (record: ConditionRecord): boolean => {
    ...
    const { runs, currentCount } = record.conditions.reduce((acc: result, curr): result => {
