export class HeapQueue<T> {
    ...
    private heapify(fromIndex: number) {
        ...
        if (leftChildIx < this.currentSize && this.lessThan(this.heap[leftChildIx], this.heap[smallest])) {
            smallest = leftChildIx
        }

        if (rightChildIx < this.currentSize && this.lessThan(this.heap[rightChildIx], this.heap[smallest])) {
